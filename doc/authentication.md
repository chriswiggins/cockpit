...
The command specified in `SessionUserCommand` will be called with two arguments, which are the authentication data extracted from the Authorization header (excluding the auth scheme prefix), and the remote host IP address. For example, if a curl command like this is issued:
...
`/usr/local/bin/my-auth-method-script some-auth-data <remote-host-ip>`
